...
1. A la racine de votre projet, exécutez la commande suivante :
    ...
    S'il vous est demandé si vous préférez placer le package dans `require-dev`, vous pouvez répondre `yes`. Cela permet de différencier dans le `composer.json` les dépendances liées aux fonctionnement global de l'application (celles de la section `require`) et celles exclusivement liées à la phase de développement, au test, etc...(comme `phpunit`). La commande `composer install` installe toutes les dépendances, mais si on utilise l'option `--no-dev`, seules les dépendances de `require` seront installées.
...
6. Exécutez le projet en choissisant la configuration `Tests unitaires` (bouton "play" en haut à droite). Vous devriez obtenir un message vous informant qu'aucun tests n'a été éxécuté (c'est normal, pour le moment!)
